...
class Game(Data):
    ...
    def tagINIT(self, tag, data):
        ...
        self.round.hands = tuple(
            self.decodeList(data[hand], Tile) for hand in self.HANDS if hand in data and data[hand]
        )
        ...
        self.round.points = [int(p) for p in data["ten"].split(",")]  # 追加
    ...
    def default(self, tag, data):
        ...
        # リーチを追加
        elif "REACH" in tag and data["step"] == '1':
            reach = Riichi(self.round.events)
            reach.player = int(data["who"])
        else:
            pass
